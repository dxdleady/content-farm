...
A ref is a style reference image in `refs/style/` (currently `ref-01.jpg` …
`ref-28.jpg`, note: `ref-13.png`). The pool is shared across products — a look is
not a brand. It carries a *look* — a medium,
palette, lighting, and signature treatment — that gets transferred onto new
subjects via image-to-image.

Each ref has a **feature-map** in `refs/analysis/ref-NN.json` (2-digit,
zero-padded). `refAnalysisFile(n)` in `src/plan.ts` maps a number → filename.
...
2. Write its feature-map to `refs/analysis/<slug-or-ref-NN>.json`. Look at
   the image first, then describe the medium per the schema above. Match the tone
   of existing files (`refs/analysis/ref-01.json` … `95ae8841…json`).
3. If you use the `ref-NN` naming, `compose --ref NN` finds it automatically.
   Otherwise pass the exact analysis filename where a ref is expected.
4. Preview it: `node tools/ref-slides.ts NN NN` renders 2 sample slides.
...
On art-capable slides in a product's `RUBRICS` (`products/<id>/copy/rubrics.ts`),
`art: { s, c, k }`:
...
with the REPLACE header and `ART_DIRECTIVE`. `tools/compose.ts` also appends a
...
`src/fx.ts` exports `FX_FILTER` (an SVG `feTurbulence` grain) and `fxPage()`.
`compose.ts` grains the **photo only** (before compositing the slide), so type
