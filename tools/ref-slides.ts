...
//   node tools/ref-slides.ts 1 10        # refs 1..10
//   node tools/ref-slides.ts 1 28 --theme dark
...
import type { ArtPrompt, RefAnalysis, RenderSlide } from '../src/types.ts';
...
const W = FMT.w, H = FMT.h, HANDLE = P.handle;
...
const OUT = join(ROOT, `out/runs/ref-slides${productTag(P)}${formatTag(FMT)}`); mkdirSync(join(OUT, 'cards'), { recursive: true });
const buildPrompt = (keep: string[], art: ArtPrompt) => composePrompt(keep, [`SUBJECT: ${art.s}.`, `COMPOSITION: ${art.c}.`, `COLOUR: ${art.k}.`,
  ...(FMT.framing ? [`${FMT.framing}.`] : [])]);

// build the job list (ref × 2 templates)
type Job = { n: number; ti: number; analysis: RefAnalysis; refFile: string; refBytes: Buffer;
             t: { copy: Record<string, unknown>; art: ArtPrompt }; bg?: string };
const jobs: Job[] = [];
for (const n of refs) {
...
await pool(jobs, Number(process.env.CONCURRENCY || 4), async (j) => {
  ...
  for (let a = 1; a <= 2; a++) {
    try { const buf = await MODELS[model as keyof typeof MODELS].call({ prompt, refs: [j.refFile], ratio: FMT.ratio }); writeFileSync(cache, buf); writeFileSync(bg, buf); j.bg = bg; spent += MODELS[model as keyof typeof MODELS].price; console.log(`  ✓ ref ${j.n} · ${j.ti + 1}`); return; }
    catch (e) { if (a === 2) console.log(`  ✗ ref ${j.n} · ${j.ti + 1}: ${(e as Error).message.slice(0, 80)}`); else await new Promise(r => setTimeout(r, 4000)); }
  }
});
...
try {
  ...
  for (const j of jobs) {
    ...
    writeFileSync(join(OUT, 'cards', `s.html`), page(renderSlide(s as RenderSlide)));
    writeFileSync(png, await chrome.shootPooled(`file://${join(OUT, 'cards', 's.html')}`, W, H));
  ...
  const enc = (pth: string) => 'file://' + encodeURI(pth).replace(/#/g, '%23');
  const rowHtml = ([n, r]: [number, { ref: string; name: string; a: string; b: string }]) => `<div style="display:grid;grid-template-columns:${TH}px ${TH}px ${TH}px 240px;gap:${GAP}px;margin-bottom:${GAP}px;align-items:start">
