...
//   node tools/model-bakeoff.ts [--models=a,b,c] [--seed=n]
...
const split = (c: string) => { const [m, brief = 'full'] = c.split(':'); return { m, brief }; };
...
const rng = (s: string) => { const h = createHash('sha256').update(String(s)).digest(); let i = 0; return () => h[(i = (i + 5) % h.length)]! / 255; };
const pick = <T,>(l: T[], r: () => number): T => l[Math.floor(r() * l.length) % l.length]!;
const shuffle = <T,>(input: T[], r: () => number): T[] => { const a = [...input]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(r() * (i + 1)); [a[i], a[j]] = [a[j]!, a[i]!]; } return a; };

type Brief = { subject: string; framing: string; colorName: string; color: string };
const prompt = ({ subject, framing, colorName, color }: Brief, brief = 'full') => [
...
const briefs: Array<Brief & { ref: string }> = files.map((f, i) => {
  const r = rng(`${f}|${seed}`);
  // styles.json keeps a `$note` string alongside the wildcard pools; only pools are named here.
  const wcPool = (k: string) => S.wildcards[k] as string[];
  return {
    ref: f,
    subject: SUBJECTS[i % SUBJECTS.length]!,
    framing: [pick(wcPool('angle'), r), pick(wcPool('crop'), r)].join('; '),
    ...((([n, c]: [string, string]) => ({ colorName: n, color: c }))(pick(ACCENTS, r))),
  };
});

mkdirSync(OUT, { recursive: true });
let spent = 0;
// one column per model, one entry per brief, in brief order
...
for (const col of picked) {
  ...
  if (!MODELS[m as keyof typeof MODELS]) { console.log(`skip unknown model ${m}`); continue; }
  grid[col] = await pool<Brief & { ref: string }, string | null>(briefs, CONCURRENCY, async (b) => {
    const t = Date.now();
    ...
    try {
      const buf = await MODELS[m as keyof typeof MODELS].call({ prompt: prompt(b, brief), refs: [join(REFS, b.ref)] });
      writeFileSync(p, buf);
      spent += MODELS[m as keyof typeof MODELS].price;
      console.log(`  ${col.padEnd(18)} ${b.ref.slice(0, 8)}  ok  ${((Date.now() - t) / 1000).toFixed(0)}s`);
      return p;
    } catch (e) {
      console.log(`  ${col.padEnd(18)} ${b.ref.slice(0, 8)}  FAIL ${(e as Error).message.slice(0, 90)}`);
      return null;
    }
  });
}
...
const W = 230;
const cell = (p: string | null | undefined) => p ? `<img src="file://${p}" style="width:${W}px;display:block">` : `<div style="width:${W}px;height:${W * 1.25}px;background:#222"></div>`;
const html = `<!doctype html><html><body style="margin:0;background:#141414;color:#999;font:11px -apple-system">
<table style="border-collapse:collapse">
<tr><td style="padding:8px;color:#fff">REF</td>${Object.keys(grid).map(c => `<td style="padding:8px;color:#fff">${c}<br><span style="color:#666">$${MODELS[split(c).m as keyof typeof MODELS].price}</span></td>`).join('')}</tr>
${briefs.map((b, i) => `<tr>
