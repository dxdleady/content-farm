...
type CatRow = [layout: string, purpose: string, sample?: Record<string, unknown>];
const CAT: CatRow[] = [
...
const OUT = join(ROOT, `out/runs/${process.env.RUN_ID||'layout-catalogue'}${productTag(P)}${formatTag(FMT)}`);
mkdirSync(join(OUT,'cards'),{recursive:true});
const ONLY = process.env.CARD || null;   // CARD=stat renders just that layout
const chrome = await Chrome.launch();
const meta: Array<{id:string;layout:string;name:string;purpose:string}> = [];
try {
  ...
  for (let i=0;i<CAT.length;i++){
    ...
    const s={ layout, minimal:true, ground:G, accent:A, index:(i%8)+1, total:8, handle:P.handle, ...(sample||{}) };
    const html=page(renderSlide(s as RenderSlide));
    const id=`${String(i+1).padStart(2,'0')}-${layout}`;
    const hp=join(OUT,'cards',`${id}.html`); writeFileSync(hp,html);
    writeFileSync(join(OUT,'cards',`${id}.png`), await chrome.shootPooled(`file://${hp}`,W,H));
