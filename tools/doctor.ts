...
let failed = 0;
const ok = (label: string, note?: string) => console.log(`  ✓ ${label}${note ? `  ${note}` : ''}`);
const bad = (label: string, fix: string) => { failed++; console.log(`  ✗ ${label}\n      → ${fix}`); };
const warn = (label: string, note: string) => console.log(`  ! ${label}\n      → ${note}`);
