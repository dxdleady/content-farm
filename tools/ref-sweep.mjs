...
//
//   node tools/ref-sweep.mjs [count] [--seed=<n>]
...
for (const [i, refName] of chosen.entries()) {
  ...
  const framing = [pick(S.wildcards.angle, rr), pick(S.wildcards.crop, rr), pick(S.wildcards.lens, rr)].join('; ');
  ...
  const png = await background(null, {
