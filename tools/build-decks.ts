...
import type { Slide, Topic } from '../src/types.ts';
...
function palette(deckName: string): string[] {
...
const replace = (i: number, subject: string, colour: string) => [
  `SUBJECT: ${subject}.`,
...
function build(t: Topic) {
  ...
  // Topic.subjects and palette() both yield exactly 8 entries, one per slide, which is
  // what the literal array above is built from.
  ].map((sl, i) => ({ ...sl, replace: replace(i, s[i]!, colours[i]!) }));

  const deck = { deck: t.deck, refAnalysis: t.ref, model: 'gpt-image-2',
                 handle: P.handle, slides };
  writeFileSync(join(DECKS, `deck-${t.deck}.json`), JSON.stringify(deck, null, 2));
  return t.deck;
}

const TOPICS = JSON.parse(readFileSync(join(DECKS, 'topics.json'), 'utf8'));
for (const t of TOPICS) console.log('deck-%s.json  <- %s', build(t), t.ref.slice(0, 8));
